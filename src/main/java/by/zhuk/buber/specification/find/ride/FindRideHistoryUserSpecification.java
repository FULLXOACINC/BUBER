...
public class FindRideHistoryUserSpecification implements FindSpecification<Ride> {
    private static final String SELECT_USER_RIDE_HISTORY = "SELECT ride_index,ride_driver_login,ride_start_lat_coordinate,ride_start_lng_coordinate,ride_end_lat_coordinate,ride_end_lng_coordinate,ride_price,user_phone_number,user_name,user_second_name,ride_data FROM buber_db.ride INNER JOIN buber_db.user ON buber_db.ride.ride_driver_login=buber_db.user.user_login WHERE ride_passenger_login=? AND ride_is_driver_end_accept=1 AND ride_is_driver_start_accept=1 AND ride_is_passenger_end_accept=1 AND ride_is_passenger_end_accept=1";
